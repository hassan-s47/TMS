     ...
     public bool insert(order[] sorder)
        {
            ...
            string query = "insert into entry values("+saleno+",'"+type+"',"+totalamount+","+partyno+",'"+desc+"','"+date+"')";
            log.open();
                if(log.insertquery(query))
            {
                for(int i=0;i<rowcount;i++)
                {
                    query = "insert into sale values("+sorder[i].no+",'SALE','"+sorder[i].code+"',"+sorder[i].amt+","+sorder[i].kg+","+sorder[i].pc+","+sorder[i].bag+","+sorder[i].rate+")";
                    if (!log.insertquery(query))
                        return false;
        ...
        {
            log = new TMS.db();
            string query = "delete from entry where eno=" + key + " and etype='SALE'";
            log.open();
        ...
        public int getsaleid()
        {
            ...
            log = new db();
            log.open();
            string query= "select ISNULL(CAST(max(eno)+1 as int),0) from entry where etype='SALE'";
            read = log.searchquery(query);
